...
func GetSettings(ctx context.Context, userID int64) (*model.UserSettings, error) {
	query := `
		SELECT user_id, api_key, api_base, model, fetch_interval_min, updated_at
	...
	err := Pool.QueryRow(ctx, query, userID).Scan(
		...
		&s.FetchIntervalMin, &s.UpdatedAt,
	)
...
func UpdateSettings(ctx context.Context, userID int64, req model.UpdateSettingsRequest) (*model.UserSettings, error) {
	...
	// Upsert
	query := `
		INSERT INTO superread.user_settings (user_id, api_key, api_base, model, fetch_interval_min)
		VALUES ($1, $2, $3, $4, $5)
	...
	_, err = Pool.Exec(ctx, query,
		...
		current.Model, current.FetchIntervalMin,
	)
	if err != nil {
